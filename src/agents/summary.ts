...
ПРАВИЛА ВЫВОДА (пользователь видит каждый символ в реальном времени):
1. Начни с [прогресс] — напиши что анализируешь и сколько сообщений просматриваешь.
2. Каждую секцию начинай с ### Заголовок (например: ### Участники, ### Место, ### Расходы).
3. После каждой секции добавь [проверка] — одно предложение что проверил и подтвердил.
4. Пиши текст саммари под каждым заголовком.
5. НЕ используй markdown (**__, ||таблицы) в основном тексте.
6. Для таблиц — инструмент render_table. Для долгов — инструмент track_debt.
